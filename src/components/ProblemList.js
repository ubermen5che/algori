                    ...
                    problemList.map((problem) => {
                        return (
                            <CardComponent className="card m-2">
                                <Card onClick={props.onClick} id={problem._id} title={problem.title} step={problem.step} tagList={problem.tagList} />
                            </CardComponent>
                        )
                    })}
            </Container>
    ...
    return (
        <div>
            {tagList.length != 0 &&
                tagList.map((o) => {
                    return (
                        <Link onClick={() => props.onClick(o)} style={{ textDecoration: 'none' }}><TagButton name={o} /></Link>
                    )
                })}
        </div>
    )
